...
%DIF DEL _old.tex   Fri Sep 25 21:06:52 2026
%DIF ADD main.tex   Fri Sep 25 20:57:39 2026
%DIF 2-4c2-4
%DIF < %  main.tex
%DIF < %  "Intrusion Detection for IoT Traffic in O-RAN Edge Data Centres:
%DIF < %   Why Accuracy Alone Is Not Enough"
%DIF -------
%  main.tex -- revised manuscript, revision of 2026-09-24 %DIF > 
%  "What Held-Out Accuracy Predicts About Deploying Intrusion Detection %DIF > 
%   in O-RAN" (round 2, 2026-09-25; title without colon from 2026-09-25) %DIF > 
%DIF -------
%
...
%DIF -------
%  Every number in this document is measured. Tables arrive by \input from %DIF > 
%  tables/generated/ (analysis/make_tables_v2.py) and every number in prose %DIF > 
%  is a macro from tables/generated/numbers.tex (analysis/make_numbers.py). %DIF > 
%  Nothing numeric is typed by hand. Regenerate with: %DIF > 
%     python analysis/revision_stats.py %DIF > 
%     python analysis/make_numbers.py %DIF > 
%     python analysis/make_tables_v2.py %DIF > 
%     python analysis/make_figures_v2.py %DIF > 
%DIF -------
%
%DIF 18-23c17-19
...
%DIF < %  -------------------------------------------------------------------
%DIF -------
%  Layout: IEEEtran journal mode. For submission, move the content into the %DIF > 
%  official IEEE Access class (ieeeaccess.cls, IEEE Author Center or the %DIF > 
%  Overleaf IEEE Access template) and add author biographies. %DIF > 
%DIF -------
%=======================================================================
...
%DIF -------
% ieeeaccess.cls (official template of 2026-05-13, in paper/access/) redefines %DIF > 
% the TeX primitive \year as a one-argument macro, which breaks pgf/TikZ at load %DIF > 
% time. Keep the primitive and restore it after the class; \year{...} is only for %DIF > 
% the publisher's production staff. %DIF > 
\let\TeXyear\year %DIF > 
\documentclass{ieeeaccess} %DIF > 
\let\year\TeXyear %DIF > 
% The class also defines \textbf#1 as {\bf #1} (and \textit likewise), which %DIF > 
% fails on math inside the argument ("Extra }, or forgotten $"). Restore the %DIF > 
% LaTeX kernel definitions; plain text renders identically. %DIF > 
\DeclareTextFontCommand{\textbf}{\bfseries} %DIF > 
\DeclareTextFontCommand{\textit}{\itshape} %DIF > 
% The class paints its header in a Pantone spot colour registered through the %DIF > 
% page resources, which TikZ's PDF driver overwrites. Use the same colour in %DIF > 
% process CMYK (the values the class gives the spot colour). %DIF > 
\definecolor{accessblue}{cmyk}{1,0.3,0,0.2} %DIF > 
% Submission copy: the class prints a template footer ("VOLUME 11, 2023") and a %DIF > 
% "Digital Object Identifier" line in the title block. This manuscript has no %DIF > 
% volume, year of publication or DOI until IEEE assigns them, so both are %DIF > 
% removed rather than left as values that look real. Production restores them. %DIF > 
\usepackage{etoolbox} %DIF > 
\makeatletter %DIF > 
\patchcmd{\ps@titlepage}{VOLUME\ \thevol, \theyear}{}{}{} %DIF > 
\patchcmd{\ps@titlepage}{VOLUME\ \thevol, \theyear}{}{}{} %DIF > 
\patchcmd{\ps@headings}{VOLUME\ \thevol, \theyear}{}{}{} %DIF > 
\patchcmd{\ps@headings}{VOLUME\ \thevol, \theyear}{}{}{} %DIF > 
\patchcmd{\@maketitle}{Digital Object Identifier\space\@doi}{}{}{} %DIF > 
\pagestyle{headings} %DIF > 
\makeatother %DIF > 
% Caption math uses the class's "medium" math version, whose operator font %DIF > 
% (Formata) has no upright Greek: \Delta printed as an accent in Table and %DIF > 
% Figure captions. Take \Delta from Computer Modern in every math version. %DIF > 
\DeclareSymbolFont{cmupgreek}{OT1}{cmr}{m}{n} %DIF > 
\DeclareMathSymbol{\Delta}{\mathalpha}{cmupgreek}{"01} %DIF > 
%DIF -------
...
% Clickable citations, cross-references, URLs and DOIs. The IEEE Access class %DIF > 
% loads hyperref only in its JTEHM mode; hidelinks keeps the printed page %DIF > 
% identical to the class's own look. %DIF > 
\usepackage[hidelinks,bookmarks=false]{hyperref} %DIF > 
%DIF -------
...
%DIF -------
\usetikzlibrary{arrows.meta,positioning,calc,fit,backgrounds,shapes.geometric,shapes.symbols,patterns,decorations.pathreplacing} %DIF > 
%DIF -------
...
%DIF 60a77-78
\newcommand{\DC}{\ensuremath{\mathcal{D}_{C}}} %DIF > 
\newcommand{\ind}{\ensuremath{\mathbf{1}}} %DIF > 
%DIF -------
...
\DIFdel{days of real capture time, and an independently collected deployment. Each step is closer to what a deployment experiences and each costs more than the choice of architecture does. }\DIFdelend \DIFaddbegin \DIFadd{accuracy on a random split of one corpus. We ask what such a number predicts about deployment, using NetsLab-5GORAN-IDD, 5G-NIDD, six architectures, two input-blind baselines, and a ladder of protocols. }\DIFaddend A random split \DIFdelbegin \DIFdel{inflates }\DIFdelend \DIFaddbegin \DIFadd{raises radio-layer }\DIFaddend macro-$F_1$ by \DIFdelbegin %DIFDELCMD < \measured{0.09} %%%
\DIFdel{to }%DIFDELCMD < \measured{0.17}%%%
\DIFdel{, and the inflation tracks model flexibility while the trivial baselines gain nothing, identifying memorisation as the mechanism. A }\DIFdelend \DIFaddbegin \DIFadd{\LeakAvg{} (95\% interval \LeakAvgNbLo{} to \LeakAvgNbHi{}; significant for two sequence models), and a nearest-neighbor lookup gains \NNGain{}, consistent with models recognizing capture sessions. The radio capture is ordered by scenario, so a }\DIFaddend time-disjoint split \DIFdelbegin \DIFdel{costs a further }%DIFDELCMD < \measured{0.269} %%%
...
Open Radio Access Network (O-RAN) architectures disaggregate the base station into radio, distributed\DIFdelbegin \DIFdel{and centralised }\DIFdelend \DIFaddbegin \DIFadd{, and central }\DIFaddend units connected by open interfaces, and \DIFdelbegin \DIFdel{place }\DIFdelend \DIFaddbegin \DIFadd{add }\DIFaddend a RAN Intelligent Controller (RIC) \DIFdelbegin \DIFdel{alongside them to host }\DIFdelend \DIFaddbegin \DIFadd{that hosts }\DIFaddend third-party control \DIFdelbegin \DIFdel{logic \cite{polese2023understanding,abdalla2022toward,niknam2022intelligent}. In most commercial designs the }\DIFdelend \DIFaddbegin \DIFadd{applications~\cite{polese2023understanding,abdalla2022toward,niknam2022intelligent}. The }\DIFaddend near-real-time RIC \DIFdelbegin \DIFdel{is not deployed at the cell site but inside a regional edge data centre that aggregates traffic from tens to hundreds of cells \cite{shi2016edge,bonati2021intelligence}. That data centre therefore sees the union of the control-plane }\DIFdelend \DIFaddbegin \DIFadd{runs close to the cells it controls~\cite{bonati2021intelligence,polese2023understanding}, so it observes the telemetry of every device those cells serve. Compromised devices, many of them IoT devices, are a documented source of volumetric attacks~\cite{antonakakis2017mirai,kolias2017ddos}. That makes a detection application (xApp) inside the RIC control loop an attractive defense: platforms for building xApps exist~\cite{polese2023coloran,schmidt2021flexric}, }\DIFaddend and \DIFdelbegin \DIFdel{user-plane telemetry generated by every IoT device attached to those cells , which makes it a natural vantage point for detecting compromised devices before their aggregate behaviour degrades the radio network. }\DIFdelend \DIFaddbegin \DIFadd{detectors that run in or beside the O-RAN control loop have been built~\cite{wen2024spector,scalingi2024detran}.
}\DIFaddend 

\DIFdelbegin \DIFdel{The motivating threat is well documented. Large IoT botnets have repeatedly been assembled from deviceswith weak or default credentials \cite{antonakakis2017mirai,kolias2017ddos}, and in cellular deployments the damage is not limited to volumetric flooding of an external target. Bursts of attach, detach and radio resource control transitions from a synchronised population of devicesimpose a signalling load that scales with the number of state changes rather than with the number of transmitted bytes, and this class of signalling storm has been shown to degrade a cellular network well before any link saturates \cite{gorbil2016signalling}. Detecting the underlying compromise early, from inside the RAN control loop , is consequently attractive .

}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The natural implementation is a detection xApp: a containerised application that subscribes to E2 service model reports, extracts features, classifies traffic, and issues control actions through the same interface \cite{polese2023coloran,schmidt2021flexric}.
A substantial and growing literature proposes such xApps}\DIFdelend \DIFaddbegin \DIFadd{Detectors for 5G and O-RAN traffic are usually evaluated on a random held-out split of one dataset}\DIFaddend , and the \DIFdelbegin \DIFdel{headline result is remarkably consistent across papers: detection }\DIFdelend \DIFaddbegin \DIFadd{reported }\DIFaddend accuracy or $F_1$ \DIFaddbegin \DIFadd{is commonly }\DIFaddend above 99\%\DIFdelbegin \DIFdel{on a held-out split of a single dataset. What is far less consistent is any evidence that the detector would survive contact with a production network.

}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Three properties determine whether }\DIFdelend \DIFaddbegin \DIFadd{. On 5G-NIDD, one of the two corpora we use, the dataset's authors report binary accuracy of 99.85\% to 99.95\% for four of five classifiers~\cite{samarakoon2022niddarxiv}, and later work reports weighted $F_1$ of up to 99.95\%~\cite{ilias2024moe5g}. Three properties decide whether such }\DIFaddend a detector is deployable, and \DIFdelbegin \DIFdel{none of them is measured by }\DIFdelend a held-out split of the training distribution \DIFaddbegin \DIFadd{measures none of them}\DIFaddend . The first is \DIFdelbegin \DIFdel{cross-deployment generalisation. A model trained on telemetry from one radio configuration, device population and traffic mix must classify correctly in a different one, and network intrusion detection models }\DIFdelend \DIFaddbegin \DIFadd{generalization to a deployment that did not supply the training data; intrusion detectors }\DIFaddend are known to transfer poorly between independently collected datasets\DIFaddbegin \DIFadd{~}\DIFaddend \cite{layeghy2022generalisability,layeghy2023benchmarking}. The second is alert burden\DIFdelbegin \DIFdel{. A }\DIFdelend \DIFaddbegin \DIFadd{: a }\DIFaddend false positive rate that \DIFdelbegin \DIFdel{appears negligible on a class-balanced benchmark becomes an unmanageable }\DIFdelend \DIFaddbegin \DIFadd{looks small on a balanced benchmark becomes a large }\DIFaddend alert volume once it is multiplied by production flow rates and \DIFdelbegin \DIFdel{discounted by }\DIFdelend \DIFaddbegin \DIFadd{read against }\DIFaddend a realistic attack base rate\DIFdelbegin \DIFdel{, a consequence of the base-rate fallacy identified for intrusion detection more than two decades ago }\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend \cite{axelsson2000baserate,sommer2010outside}. The third is \DIFdelbegin \DIFdel{tail latency . The }\DIFdelend \DIFaddbegin \DIFadd{latency against the }\DIFaddend near-real-time \DIFdelbegin \DIFdel{RIC control loopis specified to operate between }\DIFdelend \DIFaddbegin \DIFadd{control loop, for which the O-RAN E2 specification requires support of latencies from }\DIFaddend 10\,ms \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend 1\,s\DIFdelbegin \DIFdel{\cite{oran_wg3_ricarch}, and a detector whose tail latency exceeds the budget does not merely respond slowly, it fails to close the loop at all. That specified range spans two orders of magnitude, which means a paper can reach opposite conclusions about the same measurements by choosing where in it to draw the line}\DIFdelend \DIFaddbegin \DIFadd{~\cite{etsi_e2gap}. A benchmark score measures how well a detector separates the records of one corpus; deployment asks how it behaves on traffic it was not trained on, at a realistic base rate, inside a control loop. We treat the first as evidence about the second only where an experiment shows that it is}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdel{The framing matters, because cross-deployment transfer between 5G intrusion detection corpora has been measured before, including }\DIFdelend \DIFaddbegin \DIFadd{Each of these critiques has been made before. The base-rate argument is Axelsson's~\cite{axelsson2000baserate}, the closed-world critique is Sommer and Paxson's~\cite{sommer2010outside}, and evaluation pitfalls in security machine learning have been catalogued by Arp et al.~\cite{arp2022dosdonts}, with temporal and spatial bias formalized by Pendlebury et al.~\cite{pendlebury2019tesseract}. Transfer }\DIFaddend between the two corpora \DIFdelbegin \DIFdel{used here~\cite{abraheem2026bidirectional}. }\DIFdelend \DIFaddbegin \DIFadd{we use has been measured by Abraheem and Edhirig~\cite{abraheem2026bidirectional}, and inference inside a real near-real-time RIC has been measured by Obiuwevwi et al.~\cite{obiuwevwi2026realtime}. }\DIFaddend Our contribution is not \DIFaddbegin \DIFadd{a new detector and not the observation }\DIFaddend that transfer degrades. It is \DIFdelbegin \DIFdel{that the \emph{evaluation protocol}, not the architecture, dominates every number a reader of this literature is given, and that this can be quantified. Our contributions are as follows.
...
}\DIFaddend \item We decompose decision latency \DIFdelbegin \DIFdel{and show that \textbf{feature extraction, not inference, is the dominant cost}, and that it remains dominant after an order-of-magnitude optimisation of the extractor. Comparison against a published measurement taken inside a real Near-RT RIC~\cite{obiuwevwi2026realtime} indicates that the conformance verdict is a property of the implementation rather than of the architecture }\DIFdelend \DIFaddbegin \DIFadd{on one host for three implementations, including a compiled runtime }\DIFaddend (Section~\DIFdelbegin \DIFdel{\ref{sec:results}). }%DIFDELCMD < 

%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{We provide a concise reporting checklist that we argue should accompany any O-RAN security paper claiming deployability (Section~}\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:res:latency}). We then combine generalization, alert burden, and latency into a deployability test, show how its verdict depends on its bounds, and derive reporting requirements (Sections~\ref{sec:discussion} and~}\DIFaddend \ref{sec:conclusion}).
\end{itemize}

\DIFdelbegin %DIFDELCMD < \noindent%%%
\DIFdel{\textbf{What this paper does not do.} Our latency measurements are \emph{emulated}: they are taken off-platform}\DIFdelend \DIFaddbegin \DIFadd{All latency figures are emulated}\DIFaddend , with no RIC in the path\DIFdelbegin \DIFdel{, and they support relative ordering and stage decomposition only. We make no near-real-time conformance claim and we do not report container CPU or memory footprints, because we did not measure them. Section~\ref{sec:limitations} states the remaining constraints, of which the most important is that our two corpora were produced by }\DIFdelend \DIFaddbegin \DIFadd{. The three corpora come from three }\DIFaddend different flow exporters, so every \DIFdelbegin \DIFdel{reported transfer gap is an upper bound spanning both }\DIFdelend \DIFaddbegin \DIFadd{cross-corpus gap mixes }\DIFaddend a change of deployment \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend a change of tooling. \DIFaddbegin \DIFadd{Section~\ref{sec:res:anatomy} measures how much that matters for the first two. The notation used throughout is summarized in Table~\ref{tab:symbols}.
}\DIFaddend 
...
%DIFDELCMD < %    * Marker 03 (decision latency) is EMULATED off-platform (EXP-043); it is
%DIFDELCMD < %      drawn hollow and dashed, unlike the two markers measured on corpora.
...
%DIFDELCMD < % live-RIC arm: designed, not executed
%DIFDELCMD < \draw[draw=fCor, fill=fPink, rounded corners=3pt, line width=0.7pt,
%DIFDELCMD <       dash pattern=on 2.4pt off 1.8pt] (5.80,-1.10) rectangle (9.75,0.52);
%DIFDELCMD < \node[font=\footnotesize] at (7.775,0.28) {LIVE NEAR-RT RIC PATH};
%DIFDELCMD < \node[font=\footnotesize\bfseries, text=fRed] at (7.775,-0.04) {NOT EXECUTED};
%DIFDELCMD < \node[font=\scriptsize, align=center, text=fInk] at (7.775,-0.66)
%DIFDELCMD <      {latency timed off-platform for\\feature construction and\\inference only};
%DIFDELCMD < 
...
%DIFDELCMD < \node[mE] at (12.72,0.50) {03};
%DIFDELCMD < \node[font=\footnotesize, align=left, anchor=west] at (12.98,0.50)
%DIFDELCMD <      {Decision latency\\[-1pt]{\scriptsize emulated, off-platform}};
%DIFDELCMD < 
...
%DIFDELCMD <   \node[mEs] at (10.10,-0.90) {03};
%DIFDELCMD <   \node[anchor=west] at (10.30,-0.90)
%DIFDELCMD <        {Decision latency: feature construction + inference, timed off-platform};
%DIFDELCMD < \end{scope}
%DIFDELCMD < 
...
%    * Marker 03 (decision latency) is EMULATED off-platform (EXP-043); it is
%      drawn hollow and dashed, unlike the two markers measured on corpora.
...
% live-RIC arm: designed, not executed
\draw[draw=fCor, fill=fPink, rounded corners=3pt, line width=0.7pt,
      dash pattern=on 2.4pt off 1.8pt] (5.80,-1.10) rectangle (9.75,0.52);
\node[font=\footnotesize] at (7.775,0.28) {LIVE NEAR-RT RIC PATH};
\node[font=\footnotesize\bfseries, text=fRed] at (7.775,-0.04) {NOT EXECUTED};
\node[font=\scriptsize, align=center, text=fInk] at (7.775,-0.66)
     {latency timed off-platform for\\feature construction and\\inference only};
...
\node[mE] at (12.72,0.50) {03};
\node[font=\footnotesize, align=left, anchor=west] at (12.98,0.50)
     {Decision latency\\[-1pt]{\scriptsize emulated, off-platform}};
...
\begin{scope}[font=\scriptsize]
  ...
  \node[mEs] at (10.10,-0.90) {03};
  \node[anchor=west] at (10.30,-0.90)
       {Decision latency: feature construction + inference, timed off-platform};
\end{scope}
...
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdel{with $q_{1-\varepsilon}$ the $(1-\varepsilon)$ quantile. We adopt $B=10$}\DIFdelend \DIFaddbegin \DIFadd{$q_{1-\varepsilon}(L) \le B$. The O-RAN E2 specification sets near-real-time latency requirements anywhere from 10}\DIFaddend \,ms \DIFdelbegin \DIFdel{, the lower bound of the near-real-time control loop \cite{oran_wg3_ricarch}, and $\varepsilon=0.01$, so \eqref{eq:budget} reduces to a constraint on the 99th percentile}\DIFdelend \DIFaddbegin \DIFadd{to 1\,s~\cite{etsi_e2gap}, so we report the smallest conforming budget rather than a verdict at one chosen value}\DIFaddend .
...
\label{eq:tau}
\end{equation}
\DIFadd{We propose this test as a reporting device, not as an O-RAN requirement: its bounds are operator requirements, and Section~\ref{sec:disc:predicate} states the values we use and how the verdict depends on them}\DIFaddend .
...
They also serve as a control: a classifier with no capacity to memorise cannot benefit from a leaky evaluation protocol, so if the floors gain nothing where the others gain, the mechanism is memorisation}\DIFdelend \DIFaddbegin \DIFadd{The baselines ignore their input, so any change in their score between two protocols is a change in class composition alone. Every supervised model receives the same class-imbalance policy: balanced class weights, applied as sample weights for the MLP and as \texttt{scale\_pos\_weight} for XGB. Hyperparameters are fixed, not tuned (Table~\ref{tab:hyper}): tuning on held-out source data would reward structure specific to the source, which is the effect the transfer experiments measure, and tuning on the target would leak it}\DIFaddend .
...
%DIFDELCMD < %    * its "MEASURED OFF-PLATFORM" brace spanned t_ind to t_act. Only t_feat
%DIFDELCMD < %      and t_inf were timed (EXP-043); the brace now covers those two, and the
%DIFDELCMD < %      untimed terms are dashed and labelled.
...
%DIFDELCMD < % live-RIC arm, drawn as in Figure 1
%DIFDELCMD < \draw[draw=fCor, fill=fPink, rounded corners=3pt, line width=0.7pt,
%DIFDELCMD <       dash pattern=on 2.4pt off 1.8pt] (14.95,3.15) rectangle (19.80,4.30);
...
%DIFDELCMD <      {\raisebox{-0.8mm}{\ficon{f2_watch}{3.2mm}}\ timed off-platform, one host};
...
%DIFDELCMD < %%%
\DIFdel{\textbf{No RIC was involved.} We }\DIFdelend \DIFaddbegin \DIFadd{No RIC was involved: we }\DIFaddend did not deploy the detector as an xApp, \DIFdelbegin \DIFdel{we did not }\DIFdelend run an E2 agent\DIFdelbegin \DIFdel{or a RAN stack, and we did not containerise anything. The latency figures in Section~\ref{sec:results} are \emph{emulated}: they time the decision path in a single Python process on a host with no CPU isolation.
They support per-stage decomposition and relative ordering between implementations, and they support no conformance claim whatever. A measurement of AI inference inside a real Near-RT RIC exists in the literature~\cite{obiuwevwi2026realtime} and reports figures three orders of magnitude below ours for the same model families, which is the comparison we draw rather than a substitute for it}\DIFdelend \DIFaddbegin \DIFadd{, or containerize anything, and all latency figures are emulated.
}

\DIFadd{The radio-layer protocol comparison uses 20 split seeds with two model seeds each; the other radio-layer and flow-layer experiments use 20 split seeds with one model seed; calibration uses 10 split seeds of a three-way source split into training, calibration, and test groups. The declared operational parameters are an attack prevalence $\pi = 0.002$ and a benign arrival rate $\lambda_{b} = 240{,}000$ flows per hour, both swept; for the radio layer we report false alerts per benign UE-hour. Neither is taken from an operator. The rate $\lambda_b$, about 67 flows per second, is below the several hundred flows per second that Jin et al. expect at a heavily loaded base station of a large LTE network~\cite{jin2013softcell}, so the alert volumes we report are low rather than high for an edge site that serves several cells. Latency is timed per call with a monotonic clock, with garbage collection enabled, after 1,000 warm-up calls, over 20,000 calls, on one thread per library; distribution-free confidence intervals are given for every quantile}\DIFaddend .
...
\caption{Macro-$F_1$ on the \DA{} radio layer under three protocols, mean over 20 split seeds; the gray line spans the three protocols of each model. The random split raises every supervised model and the model-free 1-NN lookup alike, and the input-blind baselines do not move. Keeping every attack category in training (category-stratified) does not remove the drop. Corrected intervals are in Table~\ref{tab:leakage}.}
\label{fig:leakage}
\end{figure}
\DIFaddend 
...
\DIFadd{The benign session in the test set decides it. Held out one at a time (Table~\ref{tab:benign} and Fig.~\ref{fig:benign}),\BenLowN{} of the ten benign sessions draw a false positive rate of at most \BenLowMax{}, while sessions \BenHighSessions{} (days \BenHighDays{}) draw \BenHighMin{} to \BenHighMax{}. The last benign session, the only one recorded during the attack period, draws \BenLateFPR{}, or \BenLateFA{} false alerts per benign UE-hour, but two sessions from the first week draw as much. With ten benign sessions, a change over time cannot be separated from this heterogeneity. The practical consequence is that a detector's false positive rate on a new site cannot be read off its rate on the benign sessions it was tested on}\DIFaddend .
...
\DIFdel{. Every non-trivial architecture degrades, and all six differences survive Holm correction across the family. That is the less informative half of the result}\DIFdelend \DIFaddbegin \begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/generated/fig_rev_benign.pdf}
\caption{False positive rate on each benign radio session when that session alone is held out: mean (marker) and range (bar) over the six architectures, sessions in capture order. The right axis gives the same rate as false alerts per benign UE-hour (225 windows). Sessions of the same week range from no false positive to almost all windows flagged, so the session a detector meets, rather than the time of capture, sets its false positive rate.}
\label{fig:benign}
\end{figure}
...
\caption{The label conflict in \DB{}. The same flow records occur in two capture files of the two passes, labeled benign in one and UDP flood in the other; the tiles are schematic. No function of the record's content can separate the copies, which caps balanced accuracy. The record-position fields \texttt{Seq} and \texttt{Offset} differ between the copies, make every record unique, and carry the published 99.9\% result; a deployed detector never observes them.}
\label{fig:conflict}
...
\DIFdelFL{, is reported separately as a symmetry check rather than as a second headline. It distinguishes ``detectors do not transfer'' from ``}%DIFDELCMD < \DB{} %%%
\DIFdelFL{is simply harder'', and it carries a caveat the forward direction does not: }%DIFDELCMD < \DB{} %%%
\DIFdelFL{publishes no identifiers, so it has no group key, and its own held-out reference is a random split and therefore an optimistic bound. Its $\Delta_{F_{1}}$ is consequently an over-estimate. }\DIFdelendFL \DIFaddbeginFL \end{table*}
\subsection{\DIFadd{Transfer Across Corpora}}
\label{sec:res:transfer}
\DIFaddend 

\DIFdelbegin \DIFdel{We do not report a fine-tuning result. Few-shot adaptation on labelled target data is a different experiment, it is already published for these corpora~\cite{abraheem2026bidirectional}, and it relocates the problem rather than solving it: it means each deployment requires its own labelled attack data , which is exactly the cost a pre-trained detection xApp was supposed to avoid.

}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The degradation is statistically unambiguous. A paired $t$-test over }%DIFDELCMD < \measured{20} %%%
\DIFdel{split seeds rejects the null hypothesis of equal held-out and cross-deployment }\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{tab:transfer} and Fig.~\ref{fig:transfer} report transfer from }\DA{} \DIFadd{to }\DB{}\DIFadd{. In }\DIFaddend macro-\DIFdelbegin \DIFdel{$F_{1}$ for **all six** non-trivial architectures , and every one survives Holm correction across the family ($p_{\text{Holm}} \le \measured{0.0069}$). Effect sizes are large, with $d_{z}$ between }%DIFDELCMD < \measured{0.68} %%%
...
\DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{detectors remain above chance on the target, but not far above it: target balanced accuracy averages \TrTgtBAavg{}, and for \TrChanceSig{} of six architectures its corrected interval excludes 0.5 (\TrChanceSigHolm{} after Holm correction). For reference, the stratified sampler scores \TrStratFloor{} }\DIFaddend macro-\DIFdelbegin \DIFdel{$F_{1}$ from best to worst, is smaller than the distance any of them has fallen, and the ordering is not the source ordering.
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{../figures/generated/fig_rev_shift.pdf}
\caption{Covariate shift between \DA{} and \DB{} per shared column: first-order Wasserstein distance after the quantile transform fitted on source training data, Eq.~\eqref{eq:shift}. Filled markers are the exporter-robust subset, fixed before any result under it was computed; vertical lines are the two group means. The robust columns shift as much as the others, so the gap is not only a matter of how the two exporters segment flows.}
\label{fig:shift}
\DIFaddendFL \end{figure}
...
\DIFdelbeginFL \DIFdelFL{An earlier version of this study reported a feature-family ablation and drew from it the claim that radio-layer telemetry, which contributes most to in-distribution accuracy, is also the primary source of deployment-specific overfitting, so that removing it improves transfer. }\DIFdelendFL \DIFaddbeginFL \end{table}
\DIFaddend 

\DIFdelbegin \DIFdel{\textbf{That claim is withdrawn.} It was registered in advance with a falsification condition, and }\DIFdelend \DIFaddbegin \subsection{\DIFadd{A Third Corpus}}
\label{sec:res:third}

\DIFadd{To see whether these results depend on the particular pair of corpora, we added }\DC{}\DIFadd{, the flow records of a 5G core testbed~\cite{nugraha2025fivegdatasets}, as a second target. It changes the site, }\DIFaddend the \DIFdelbegin \DIFdel{condition fired: the record-level join between the flow records and the radio telemetry cannot be established from the published artefacts, because the flow table carries no time column and shares no identifier with }\DIFdelend \DIFaddbegin \DIFadd{flow exporter, and the vantage point at once, and unlike }\DB{} \DIFadd{its labels are consistent: only \ThConfShare\% of its flows sit on records that also carry }\DIFaddend the \DIFdelbegin \DIFdel{radio layer. Without that join the ablation cannot be constructed at all, and no result we could report here would be supported by }\DIFdelend \DIFaddbegin \DIFadd{opposite label, and }\DIFaddend the \DIFdelbegin \DIFdel{data.
...
\DIFdelbegin \DIFdel{Three observations follow. First, median latency is a misleading summary.Four of the six architectures sit inside a 10}\DIFdelend \DIFaddbegin \DIFadd{The implementation moves latency more than the architecture does (Fig.~\ref{fig:latency}). For LR, the DataFrame path costs \LatLRdfPfifty{}}\DIFaddend \,ms \DIFdelbegin \DIFdel{budget at $p_{50}$ and stay inside it at $p_{99}$; the two that do not --- histogram gradient boosting and the random forest --- miss it by an order of magnitude, with $p_{99}$ of }%DIFDELCMD < \measured{94.62} %%%
...
%DIFDELCMD < %%%
\DIFdel{Second, the verdict depends on where the budget is drawn, and O-RAN specifies the near-real-time loop over a range spanning two orders of magnitude . At 1}\DIFdelend \DIFaddbegin \DIFadd{in ONNX Runtime. Across architectures, the 99th percentile of ONNX Runtime lies between \LatOnnxPnnMin{} and \LatOnnxPnnMax{}}\DIFaddend \,\DIFdelbegin \DIFdel{ms nothing conforms; at 5}\DIFdelend \DIFaddbegin \DIFadd{ms on the radio layer, against up to \LatArrPnnMax{}}\DIFaddend \,\DIFdelbegin \DIFdel{ms the decision tree alone; at 10}\DIFdelend \DIFaddbegin \DIFadd{ms for scikit-learn with arrays. A ranking of architectures by latency is really a ranking of implementations, and a comparison with measurements taken on other hosts or in other languages~\cite{obiuwevwi2026realtime} says nothing about the architectures. For context only, Obiuwevwi et al. measure inference inside an OpenAirInterface and FlexRIC near-real-time RIC in the microsecond range for logistic regression and a small MLP~\cite{obiuwevwi2026realtime}, within an order of magnitude of our ONNX Runtime figures.

}

\DIFadd{For a window-level detector the decision path is window aggregation followed by inference. Aggregating one 16-record window costs \LatAggPnn{}}\DIFaddend \,ms \DIFaddbegin \DIFadd{at the 99th percentile, and }\DIFaddend the \DIFdelbegin \DIFdel{tree, logistic regression, XGBoost and the MLP; at 100}\DIFdelend \DIFaddbegin \DIFadd{upper bound of the 99th percentile of aggregation plus ONNX inference is at most \LatEtoEPnnHiMax{}}\DIFaddend \,ms \DIFdelbegin \DIFdel{all but the random forest; at 1}\DIFdelend \DIFaddbegin \DIFadd{across the architectures with a verified export. On this host that path would meet any budget in the specified range. A repeat of the radio measurement on the same host, which kept every call for Fig.~\ref{fig:latency}, gave medians \LatRepMedRatioMin{} to \LatRepMedRatioMax{} times those of Table~\ref{tab:latency} (median ratio \LatRepMedRatio), so per-call latency on a host without CPU isolation depends on the state of the host as well as on the model; the upper 95\% bound of the 99th percentile of aggregation plus ONNX inference was still at most \LatRepEtoEPnnHiMax}\DIFaddend \,\DIFdelbegin \DIFdel{s everything. Asserting a single budget converts an authoring choice into an apparent measurement , so we report the crossing point per architecture instead.}\DIFdelend \DIFaddbegin \DIFadd{ms. Whether a deployment does is decided by the terms we did not measure: indication decoding, control encoding, and queueing inside a RIC.

}\DIFaddend 

\DIFdelbegin \DIFdel{Third, and least expected, inference is not the bottleneck.

Packet-level feature extraction costs a median }%DIFDELCMD < \measured{6.90}%%%
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{../figures/generated/fig_rev_latency.pdf}
...
\DIFdelbegin \section{\DIFdel{Measured Results}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
...
convention in much of the intrusion-detection literature, and once under a split in which whole capture runs are held out.}\DIFdelend \DIFaddbegin \DIFadd{\emph{Data path.} E2SM-KPM delivers aggregated measurements per reporting period; it does not carry packets. A window-level detector can run inside the RIC on KPM reports, but a flow-level detector needs a flow exporter next to the user plane, in the central unit or at the user-plane function, and a transport of its records to the xApp, for example through a custom E2 service model or the non-real-time data collection path. That transport adds to $t_{\mathrm{ind}}$ in Eq.~\eqref{eq:latency}, which we did not measure.
}\DIFaddend 
...
\DIFdelend \DIFaddbegin \DIFadd{\emph{Detection delay.} Compute latency is a small part of the time to detect. A window-level decision cannot precede the end of its window (16\,s here), }\DIFaddend and \DIFdelbegin %DIFDELCMD < \measured{1.35}%%%
\DIFdelend \DIFaddbegin \DIFadd{a flow-level decision cannot precede the export of the flow record, which for a long connection waits for an idle or active timeout. The KPM reporting period and the flow timeouts should be reported beside compute latency}\DIFaddend .
...
published random-split figure by a fixed amount}\DIFdelend \DIFaddbegin \DIFadd{\emph{Acting on alerts.} An alert that drives an E2SM-RC action, such as isolating a UE or steering it, acts on a false alert as readily as on a true one. At the false positive rates of Table~\ref{tab:pooled}, automatic mitigation on the radio layer would act against each benign UE \RadFAmin{} to \RadFAmax{} times per hour; alerts of this quality can inform an operator or a non-real-time policy through A1, but should not trigger automatic control}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Applying the Deployability Test}}
\label{sec:disc:predicate}
\DIFaddend 

\DIFdelbegin \DIFdel{Criterion 2 asks what the detector costs an operator rather than how itscores on a corpus. Table~\ref{tab:m_alerts} reports both quantities side by
side at the default threshold, under the declared deployment parameters
$\pi=0.002$ and $\lambda_{b}=240{,}000$ benign flows per hour}\DIFdelend %DIF >  Discussion: applying the deployability test of Eq. (7). Numbers are macros
%DIF >  from tables/generated/numbers.tex; the table is generated (EXP-052).
\DIFaddbegin \DIFadd{We instantiate Eq.~\eqref{eq:deploy} with the following bounds. They are our choice: no O-RAN specification fixes them, and an operator would set them from its own traffic and staffing. The generalization bound is $\delta = 0.10$ of balanced accuracy, applied to the upper end of the corrected 95\% interval. The alert term asks for a threshold that detects at least half of the attacks ($r_{\min} = 0.5$), makes at least one alert in ten a true one ($\rho = 0.1$ at $\pi = 0.002$), and raises at most $A_{\max} = 200$ false alerts per hour at $\lambda_{b} = 240{,}000$ benign flows per hour. All alert conditions are evaluated on }\DB{}\DIFadd{, because an operator deploys on the target, not on the source. The latency term uses the strictest budget the specification allows, $B = 10$\,ms: the upper bound of the 99th percentile of the radio decision path meets 1\,ms for every architecture (Section~\ref{sec:res:latency}), so the term passes anywhere in the specified range, with the caveat that indication decoding, control encoding, and queueing were not measured}\DIFaddend .
...
control. Sweeping the decision threshold to recover precision, Table~\ref{tab:m_ppvcross} shows that $\text{PPV}=0.5$ is
\emph{unreachable at any threshold} for four of the five detectors. The scores of the boosted models and the perceptron saturate near unity under
a run-disjoint split, so raising $\tau$ barely moves the alert volume: the histogram-boosted ensemble still emits }%DIFDELCMD < \measured{53{,}632} %%%
...
\DIFdelbeginFL %DIFDELCMD < \caption{Decision threshold required to reach a usable operational
...
%DIFDELCMD < %%%
\DIFdel{O-RAN specifies the near-real-time control loop over a range of 10\,ms to
...
choice. Table~\ref{tab:m_latency} and Fig.~\ref{fig:m_latency} therefore report, for each architecture , the smallest budget its 99th percentile conforms to. }%DIFDELCMD < 
...
\DIFdel{\,ms four of six conform. At }%DIFDELCMD < \measured{1000}%%%
\DIFdel{\,ms all of them
do. One set of measurements thus supports four different conclusions, separated only by where the line is drawn, which is the reason to report
the crossing point instead of a pass-fail column. }%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{../figures/generated/fig_rev_sensitivity.pdf}
...
\DIFdel{to }%DIFDELCMD < \measured{0.96}%%%
\DIFdel{\,ms for the four architectures that conform at 10\,ms, whereas constructing features from packets costs a
...
histogram-boosted ensemble, whose own inference exceeds extraction, so the observation is architecture-dependent and is stated as such.
}%DIFDELCMD < 
...
implementation would be substantially faster; the defensible claim is therefore that feature construction is of the same order as or larger than
...
reported beside the median}\DIFdelend \DIFaddbegin \DIFadd{\emph{Is the test attainable?} A test that no detector passes could simply be too strict. The published 5G-NIDD pipeline of Section~\ref{sec:res:target} shows that it is not. On its own random split, with the test's alert term evaluated on that split, \PubRzPassN{} of its five classifiers pass: operational precision reaches \PubRzPPVmax{} at $\pi = 0.002$ with as few as \PubRzFAmin{} false alerts per hour at $\lambda_b = 240{,}000$. The same pipeline fails the test one rung later, when the two position counters are removed, and at every later rung, on all flows and on the flows without the benign copies (Table~\ref{tab:ladder}). The test is therefore only as informative as the evaluation it is applied to: applied to a random split that still contains record positions, it certifies the split, and the pass disappears as soon as the evaluation moves one step toward deployment conditions. The detectors of this paper fail it on the evaluations closest to deployment that the corpora allow}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdel{We regard this as the honest outcome rather than a negative result to be buried. Reporting only Table~\ref{tab:indist} under a random split would have supported a claim of a deployable high-accuracy detection xApp, and that claim would have been wrong in three independent ways. }%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{\textbf{The grouping key is confounded with attack identity, and unequally across layers.} Grouping the radio layer by capture sessionis perfectly aligned with attack category --- purity $1.000$ --- because each run exercises one scenario, so no radio-layer result can distinguish memorising a run from memorising an attack . Network-layer grouping by source address reaches only $20.6\%$ of an oracle that groups directly by attack type, so the two are separable there.
A run-disjoint label should not be quoted without the corresponding purity figure.

}\DIFdelend \DIFaddbegin \DIFadd{\emph{Radio capture schedule.} Nine of the ten benign radio sessions were recorded six weeks before any attack session, mostly with one UE, and every radio label is a session label. A radio detector may learn to separate capture periods as well as attacks. The one benign session from the attack period is flagged almost entirely, but so are two early sessions (Table~\ref{tab:benign}), and ten benign sessions cannot separate the two explanations.
}\DIFaddend 

\DIFdelbegin \DIFdel{\textbf{The effective sample size is smaller than it appears.} Twenty split seeds resample thirty capture sessions; they do not enlarge that population. Intervals are paired $t$-intervals over split means rather than bootstraps for this reason, and the temporal analysis is powered to detect a large effect and nothing subtler. }\DIFdelend \DIFaddbegin \DIFadd{\emph{Three corpora.} Three corpora establish that the gap recurs, not how it varies across deployments. The third is a 5G core testbed rather than an O-RAN deployment, holds only floods and a control-plane attack, and has three capture files, too few for cluster intervals. No third O-RAN corpus with paired radio telemetry was available to us.
}\DIFaddend 

\DIFdelbegin \DIFdel{\textbf{Three attack categories cannot be tested in transfer.} }%DIFDELCMD < \DB{} %%%
\DIFdel{contains no DDoS, brute-force or web attacks, so those categories are reported as untestable rather than averaged into a macro score as though they had been evaluated. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{\textbf{Two corpora are two points.} They establish that a gap exists, not how it varies across the space of deployments}\DIFdelend \DIFaddbegin \DIFadd{\emph{Latency.} All latency figures are emulated on one Windows host without CPU isolation, and $t_{\mathrm{ind}}$, $t_{\mathrm{act}}$, and $t_{q}$ are not measured. CPU and memory use under load are not measured}\DIFaddend .

\DIFdelbegin \DIFdel{The operational prevalence $\pi=0.002$ in \eqref{eq:ppv} is a declaredassumption, not a measurement. We therefore report the sweep rather than any single value, and we note that PPV }\DIFdelend \DIFaddbegin \DIFadd{\emph{Operational parameters and the test.} The prevalence $\pi = 0.002$ and the benign arrival rate $\lambda_b$ are declared, not measured on any network; we sweep both, and operational precision }\DIFaddend depends on $\pi$ \DIFdelbegin \DIFdel{while alert volume scales linearly and independently in the benign arrival rate, so the two-dimensional parameter space collapses to one dimension plus a multiplier.
The prior correction of \eqref{eq:prior} assumes $\pi$ is known, which in deployment it is not; where we apply it with a known target prior we label the result an oracle and draw no claim from it .

}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Finally, we evaluate supervised binary detectiononly. Unsupervised baselines such as isolation forests \cite{liu2008isolation}, feature-attribution analysis \cite{lundberg2017shap}and multi-class attack attribution are left to future work, and }\DIFdelend \DIFaddbegin \DIFadd{alone. The bounds of the deployability test are our choice (Section~\ref{sec:disc:predicate}). Fig.~\ref{fig:sensitivity} gives the verdict under other bounds, but an operator may weigh recall, precision, and alert volume differently, and }\DIFaddend the \DIFdelbegin \DIFdel{last of these would likely transfer worse still}\DIFdelend \DIFaddbegin \DIFadd{test does not price detection delay or the cost of a missed attack.
}
...
\DIFdelbegin \DIFdel{An intrusion detection xApp evaluated the usual way tells you very }\DIFdelend \DIFaddbegin \DIFadd{A held-out score on one corpus says }\DIFaddend little about how \DIFdelbegin \DIFdel{it will behave anywhere else. On an independently collected deployment, every architecture we tested degrades significantly, the best of them clears a stratified coin flip by }%DIFDELCMD < \measured{0.148} %%%
\DIFdel{of macro-$F_{1}$, and the architecture that scored \emph{highest} on held-out source data falls \emph{below} that floor, with a balanced accuracy of }%DIFDELCMD < \measured{0.4966}%%%
\DIFdel{. Across six architectures there is no significant rank correlation between in-distribution and transfer performance. At the declared deployment base rate}\DIFdelend \DIFaddbegin \DIFadd{a detection xApp behaves elsewhere. On }\DIFaddend the \DIFdelbegin \DIFdel{same detectors carry corpus precision near }%DIFDELCMD < \measured{0.93} %%%
...
%DIFDELCMD < %%%
\DIFdel{None of this is visible in the number a paper usually reports. That is the argument of this work, and }\DIFdelend \DIFaddbegin \DIFadd{corpora studied here, a random split inflates the score, and a model-free lookup gains as much, which points to the recognition of capture sessions. The radio corpus's capture schedule leaves no room to estimate drift, and }\DIFaddend the \DIFdelbegin \DIFdel{strongest evidence for it is that the argument applies to us. Two findings we previously published did not survive re-examination during this study. A reported sixfold spread in deployment precision between architectures was an artefact of averaging a non-linear functional across evaluation folds rather than pooling counts; pooled, the spread is }%DIFDELCMD < \measured{1.40}%%%
