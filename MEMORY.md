...
- 2026-09-26 (Adeel: reviewers asked for academic writing, and the previous humanize pass read as AI). Full prose rewrite into conventional journal register: abstract, introduction, related work, criteria, methodology, setup, all Results subsections, discussion, reproducibility, limitations, conclusion, latency_section.tex, predicate_section.tex, and seven figure captions. Removed the AI tells the earlier pass had added or kept: one-line punch sentences ("The floors do not move.", "Macro-F1 is the wrong instrument here.", "The benign session in the test set decides it."), colon reveals, "X, not Y" antitheses, staccato rhythm. "therefore" 35 -> 14, "rather than" 16 -> 11. Mean sentence length 28 words. Subsection "What the Transfer Gap Is Made Of" -> "Composition of the Transfer Gap"; run-in heads "Is the test attainable?" -> "Attainability of the test.", "A published result down the ladder" -> "under the same ladder". Fixed a broken sentence order in latency_section ("Whether a deployment does ..."). Causal wording kept hedged ("consistent with the recognition of capture sessions").
- Checks: scripted comparison against HEAD shows no number macro, literal number, citation, label or cross-reference lost or added. Abstract 248 words. Build 21 pages (was 19; longer sentences), 0 errors, 0 bibtex warnings; withdrawn-claims and placeholder guards OK; reproduce.py paper 37 of 37 identical; pytest 61/61. build_diff.py now excludes \emph from inline markup (latexdiff broke on the renamed run-in heading); diff.pdf 29 pages, 0 errors. REPRODUCE.md page count 19 -> 21.
- 2026-09-26 02:00 onward (Adeel: are the open issues important, solve them all; combine academic register with the writing-style skill). Triage: live RIC (important, done as EXP-061), exporter confound (important, EXP-063), 5G-NIDD label correctness (important, resolved by EXP-062 with addresses; email draft for the authors in reports/5gnidd_label_query_draft.md, not sent), pi = 0.002 source (made moot with a break-even prevalence), ICCCN spellings (checked: Crossref, Semantic Scholar and OpenAlex raw strings all say Nannou and Ur Rahman; bib unchanged), 30 radio sessions (inherent, stays a limitation), corresponding-author email (author item, not touched), page count (optional, unchanged at 21).
- EXP-061, live RIC loop. scripts/ric/01_setup_toolchain.sh (cmake/ninja/swig from PyPI, libsctp and pcre2 unpacked from Ubuntu .debs, no sudo), 02_build_flexric.sh (FlexRIC v2.0.0 1f04cc55, Release, XAPP_MULTILANGUAGE=OFF: SWIG 4.5 emits Python 2 PyInt_* calls), 03_smoke_test.sh (nearRT-RIC + emu_agent_gnb + xapp_kpm_moni), export_radio_models.py (the six EXP-043 radio models as verified ONNX, 589 held-out windows, reference scores; one window differs from load_radio in the rate features, recorded), xapp_ids_timing/ (C xApp: KPM subscription with 16 items per UE, per-UE aggregation + ONNX Runtime 1.30.0 C API, RC QoS-flow-mapping control per report, queue + worker thread; t_ind from the KPM header timestamp, which FlexRIC truncates to 32 bits of microseconds, handled modulo 2^32), 04_build_xapp.sh, 05_run_exp061.sh, run_exp061.py (one command from Windows). analysis/ric_latency.py.
- EXP-061 results (run alone): 6 models x 5,000 reports at 10 ms, LR and RF x 20,016 at 1 ms, 0 dropped, C = Python scores within 3.6e-7. Loop p99 (all UEs, indication to CONTROL-ACK) 4.45-4.95 ms at 10 ms (upper bound <= 5.01), 1.42-1.69 ms at 1 ms; t_ind p99 3.05 ms, t_act 1.50 ms, inference <= 0.23 ms; delivery + control >= 86% of mean loop time. Every term faster at 1 ms (host idle states, not controlled; stated). Max single value 11.2 ms at 10 ms.
- Paper for EXP-061: Eq. (5) terms redefined (delivery, control to ACK, queueing in the xApp); setup paragraph on FlexRIC/xApp; new Table tab:ric (rev_ric.tex, generated), results paragraphs in latency_section.tex; predicate B_min column now from the RIC loop (revision_stats ric_min_budget_ms); intro scope sentence and contribution; limitations; conclusion; requirement (vi) now "through a RIC and, before a conformance claim, with a real E2 node"; requirements are an enumerate list. Figures 1 and 2: live-RIC arm redrawn as measured (solid teal, "MEASURED", emulated E2 node named), marker 03 filled, Fig. 2 runtime stages all timed with p99 labels. Algorithm paragraph no longer says "not deployed in a RIC".
- pi: make_numbers PredPiStar (0.076, 38x the declared value; 0.005 without the copies) = prevalence at which the best operating point would reach rho; predicate text says the verdict does not depend on pi because the false-alert count (>= 5,603/h clean, 95,507/h all) fails A_max at any pi.
- EXP-062, label addresses: scripts/fetch_5gnidd_file.py (Fairdata/Etsin v3 authorize + download.fairdata.fi; Git Bash rewrites "/x" args, so names are passed without the slash), analysis/label_conflict_addresses.py. BTS1_BTS2_fields_preserved.csv aligns with Encoded.csv on every row. All 281,525 benign copies are UDP 10.155.15.7 -> 10.41.150.68, 03:49.0-13:32.4, the same host pair and window as the 281,529 UDPFlood flows of file 15; file 5's own attacker is 10.155.15.4 -> same victim. The BS1 capture recorded BS2's flood and only its own attacker was labeled. All 10 BS1 per-attack CSVs match capture files 1-10 (row count + attacker): the pass-to-station assignment is confirmed. Paper: label paragraph, limitation, contribution 2, conclusion; LcAddr*/LcSite* macros.
- EXP-063 (running at the time of this entry): 5G-NIDD GTP-removed pcaps (BS1 2.33 GB, BS2 1.53 GB) re-extracted with Zeek 8.0.10 LTS (scripts/zeek/01_setup_zeek.sh, OBS Ubuntu 24.04 build unpacked in user space; 02_run_zeek.sh). experiments/build_d_b_zeek.py labels Zeek flows by host pair from the published labels; the rule reproduces all 1,215,890 published Argus labels (agreement 1.000000). 346,882 Zeek flows, 76% attack, 66,033 flood copies (the duplication is in the captures themselves). Loader load_target_d_b_zeek, run_transfer_v2 direction a_to_bz, analysis/exporter_control.py (paired with EXP-056, same 10 seeds).
- Prose pass (academic register + writing-style rules): roadmap openers removed (Sections II, III, IV, VI, VII), "namely" 6 -> 0, "which corresponds to" 5 -> 0, "that is," 2 -> 0, "the present study" 5 -> 1, passive citations in the introduction made active, "Moreover/Furthermore" mostly gone, semicolon in intro/criteria/figure-1 caption split. preserve.py: no macro, citation, label, reference or literal number lost.
- EXP-063 result (894 s, 10 seeds, models identical to EXP-056: source metrics equal to 0.0): balanced accuracy with Zeek-extracted D_B 0.43-0.56 on all flows (Argus 0.54-0.62; paired diff -0.15 to -0.06, 6 of 6 below zero) and 0.41-0.79 without copies (Argus 0.61-0.78; five models -0.06 to +0.04, none significant; LR -0.20). FPR higher under Zeek for every model. A common exporter does not close the gap. Paper: new "A common exporter" paragraph in VI-E, intro scope sentence, contribution 3, Limitations "Flow exporters" rewritten (Zeek version/config of D_A undocumented; host-pair labeling caveat; D_C not re-extracted). Zk* macros.
- Abstract rewritten (label addresses, common exporter, RIC loop; the time-split and fold-averaging sentences moved out; qualifier "significant only for two sequence models" kept because the interval crosses zero): 249 source words, 250 rendered.
- experiments/final_validation.py reads EXP-061 (LATENCY and RIC INTEGRATION PASS with the emulated-node caveat): 22 PASS 0 WARN 1 BLOCKED 0 FAIL. README, REPRODUCE.md (38 of 38, EXP-061/062/063 rows), EXPERIMENTS.md, registry, reproduce.py updated. reports/finalization_audit_2026-09-25.md "Third pass".
- Checks: build 21 pages, 0 errors, 0 overfull, 0 bibtex warnings; reproduce.py paper 38 of 38 identical (manifest refrozen, +rev_ric.tex); pytest 61/61; withdrawn-claims OK; diff.pdf 30 pages 0 errors (build_diff needs an absolute --perl-lib path).
- 2026-09-26 (Adeel: GPTZero still flags the text; rewrite the whole paper with the language skill, plain wording with academic terms used sparingly, step by step). humanize-latex pass over every prose paragraph of main.tex, latency_section.tex and predicate_section.tex, by whole paragraph (scratchpad para.py replaces a one-line paragraph by its unique prefix): abstract, introduction and contribution list (items now noun phrases), related work (catalogue style replaced by argument), criteria lead-ins to the equations, methodology (the 400-word corpora paragraph split into D_A, sessions, D_B, D_C), setup, all results subsections (label-conflict and published-pipeline paragraphs split in two), latency and predicate sections, discussion, reproducibility, limitations, conclusion, and six interpretive figure-caption sentences. Table captions left as definitions. Claims checked while rewriting: first authors Engelen and Flood match the bib; "every study we know of" reverted to "repeatedly proved poor"; unsupported "flow layer is more stable" removed. "signalling" -> "signaling" (US).
- Measured effect (scratchpad burst.py, HEAD vs now): sentence-length CV 0.60 -> 0.65, adjacent similar-length sentence pairs 23% -> 19%, sentences of 35+ words 15% -> 9%, "whereas" 14 -> 2, "thus" 8 -> 2, "therefore" 10 -> 5. No detector score is claimed.
- Checks: preserve.py shows no macro, citation, label, reference or literal number lost; build 21 pages, 0 errors, 0 overfull, 0 bibtex warnings; reproduce.py paper 38 of 38 identical; withdrawn-claims OK; abstract 248 rendered words; no em dashes, no tabs.
- 2026-09-26 (Adeel: add authors from his IEEE Access submission b57487b2 in Downloads, without Muhammad Shaharyar Ramzan, plus Ali Akarma second-last; Eraj Khan corresponding; then Adeel Ahmad first author; then go under 20 pages). Author line now Adeel Ahmad, Arshad Ali, Eraj Khan, Ali Akarma, Gahangir Hossain; three affiliations and the corresponding-author line copied from that submission; Ali Akarma placed at Islamic University of Madinah (from his biography, to be confirmed). Running head "A. Ahmad et al.". Five IEEEbiography entries with photos in paper/authors/ (bios verbatim from that submission; Ali Akarma's as given by Adeel; Arshad Ali's source photo is only 90x113 px). Photos not committed yet (public repo, awaiting Adeel's decision).
- Page cut 21 -> 19: removed Fig. 2 (pipeline overview; fig2_pipeline.tex kept but no longer input), tables of sequence models, category-stratified time split, reverse transfer, shared-space cost, PPV estimators, per-session benign FPR (Fig. 5 shows the same values), notation, per-category detection, and the precision-vs-prevalence figure; hyperparameter table turned into two sentences; abbreviations now expanded at first use (KPM, LR, DT, RF, XGB, HGB, MLP, GRU, 1D-CNN, IF, AE, BA, UE); six passages tightened (reproducibility, two limitations, Platt diagnosis, two contribution bullets). Subsection IV-F renamed "Decision Loop of the xApp". Every removed float's content is stated in the text; no citation or number macro lost (only "29 sessions", moved into the Fig. 5 caption). Build 19 pages, 0 errors, 0 overfull; table and figure numbering follow first citation. Generated tables unchanged (still produced by make_tables_v2).
- 2026-09-26 (Adeel pasted a forensic writing-audit prompt). Diagnosis only, manuscript untouched: reports/forensic_writing_audit_2026-09-26.md, 151 issues (4 Critical, 41 High, 55 Medium, 51 Low) and 16 recurring patterns over main.tex, latency_section.tex, predicate_section.tex and all captions. Critical: abstract "precision never exceeds 0.063" is unscoped (D_C gives 0.092, held-out D_A 0.42); the latency term of the deployability test is measured on the radio-window loop but applied to flow detectors ("Only latency passes"); pred:25 says the published pipeline fails at R1 on clean flows, but revision_stats.py:437-439 builds verdict_clean from the all-flows generalization term, and on one label set DT and RF at R1 would pass; the generalization verdict (bounds 0.32-0.50) uses the all-flows labels the paper calls wrong. Also found: the per-10^6 false-alert figure promised at main.tex:197 is not reported anywhere; the Fig. 9 caption still names aggregation+ONNX as the test path. Compared flagged sentences with 9001d8c (for example "points to" became "exposes the mechanism").
- 2026-09-26 (Adeel pasted a forensic manuscript-repair prompt; repair against reports/forensic_writing_audit_2026-09-26.md). Report with all 151 IDs: reports/forensic_repair_report_2026-09-26.md (143 fixed, 4 merged, 3 need author input: L-01 title, L-16 Jin et al. verb, L-43 whether 5G-NIDD authors were contacted; L-35 partly). Findings that changed: (1) C-03 analysis/revision_stats.py published_ladder() now builds verdict_clean from clean_ba_drop_from_r0 (one label set); on clean flows DT and RF PASS at R1 and RF (published features) passes at R3, none at R2 -> attainability paragraph rewritten; (2) C-04 predicate() takes gen_upper, predicate_clean.csv now uses the clean dBA bound (0.29-0.41; still 0 of 6, verdict carried by the alert term); (3) reverse transfer: LR's gain is also significant (3 of 6 changes). Full revision_stats run reproduces the targeted refresh exactly.
- Generators: make_numbers.py new macros (NNChanceAny/Cat from new analysis/nn_chance.py -> results/EXP-042/processed/nn_chance.json [needs raw D_A radio data]; BenEarlyHigh*, BenMid*; TrRevLRgain, TrRevTreeLoss*; Cal BA/FPR/isotonic-sign macros; LcOwnConf 69,324 with consistency guard; PredCleanGen*; FwdEnsCleanFPR*; PubOneCleanPassModels; PubThreeNoposCleanDropMin; NetCopyFlagOthMin), NNGain from unrounded values (0.14), counts 0-9 as words, CalTempMax as 2\times10^{-5}, negative numbers wrapped in \mbox. make_tables_v2.py: Table 1 "30 sessions"/"D_A flows", Table 3 "Sess.-disj.", Table 6 RF clean BA+FPR and Pass all/clean, Table 10 "Recall"/"Corpus prec."/"w/o copies", Table 11 target balanced accuracy instead of macro-F1, Table 14 B_min dagger. make_figures_v2.py: Fig. 2 legend session-disjoint, Fig. 10b clean curve. fig1_architecture.tex: column numbers removed, KPI->KPM, "update policy"->"remap QoS flow". fig_conflict.tex: station 1/2, "reproduced". references.bib: [12] note moved to comment (arXiv v2, 2025), [14] colon.
- Prose (main.tex, latency_section.tex, predicate_section.tex): every audit item repaired (claim scope, session/split/station terminology, window stride + Algorithm 1 per completed window with in-xApp budget B_x, latency term = window-level only, flow latency not measured, Eq. (7) tau* now prose, research question in the Introduction, label-set tags on every D_B statement). Then compressed 21 -> 19 pages by content only (no format changes); abstract 248 words. Reverted my own "Jin et al. report" to "expect" (not verifiable). Checks: build 19 pages 0 errors/0 undefined/0 overfull/0 bibtex warnings; reproduce.py freeze + paper 38 of 38 identical; pytest 61/61; withdrawn-claims and placeholder guards OK; scripted diff vs pre-revision: no citation lost. Not committed.
- 2026-09-26 (Adeel: yes commit; make the GitHub link blue). paper/main.tex Section VIII: \textcolor{blue}{\url{...}} on the repository link only (other links keep hidelinks). Build 19 pages, 0 errors. Committed with the audit repair; author photos in paper/authors/ still left uncommitted (public-repo decision pending).
- 2026-09-26 (Adeel: put Ali Akarma last; do the remaining items). Author line and biographies now Ahmad, Ali, Khan, Hossain, Akarma; biographies use \authorphoto (IfFileExists), so a checkout without paper/authors/ still builds. Title and running head "What Held-Out Scores Predict About Deploying Intrusion Detection in O-RAN" (L-01), also README and REPRODUCE; README author list and BibTeX (ahmad2026oranids) brought up to date. SoftCell checked: "extrapolate from measured radio-bearer arrivals" (L-16). Fig. 8 caption reading (L-35). Ethics: repository includes the label-audit scripts (L-43, no claim about contacting the authors). Hossain Ph.D. 2014 (UNT resume); Khan's year left out (no primary source). Small trims kept 19 pages; abstract 248 words. A sed edit turned \t into a tab once; repaired, main.tex checked tab-free. Report section F updated.
- 2026-09-26 merged origin/main (Adeel removed README Limitations and Citation on GitHub; deletion kept). README findings table aligned with the repaired paper (session-disjoint, content copies, attainability after C-03, window-level latency, exporter not ruled out as sole cause, D_C leave-one-file-out reference).
- 2026-09-26 pushed to GitHub (main at 665533d after merging the README edit). Reproducibility zip rebuilt with git archive: E:/Work/New ORAN paper/oran-ids-deployability_reproducibility_package.zip. Verified from a clean extraction (no paper/authors/): reproduce.py paper 38 of 38 identical, 19 pages, 0 errors, guards OK. Zip rebuilt once more after this log entry so repo, GitHub and zip match. Author photos still not in the public repository.
- 2026-09-26 (Adeel: no need to contact the 5G-NIDD authors; put nothing about it in the paper). Removed from main.tex: the Section VIII sentence about the dataset authors checking the audit; "the dataset authors have not confirmed this reading" (VI-C) and "the dataset authors have not confirmed this" (IX) reworded as our own caveat ("our inference from the addresses", "an inference from the addresses alone"). README parenthetical removed. Repair report updated. Build 19 pages, 0 errors.
- 2026-09-26 (Adeel): affiliations corrected. [2] Faculty of Computer Information Science, Higher Colleges Of Technology, Al Mizn- Baniyas North- Abu Dhabi, United Arab Emirates (e-mail kept); [3] Anuradha and Vikas Sinha Department of Data Science, University of North Texas, Denton, TX, USA, 76203-5017. Wording exactly as Adeel gave it. Build 19 pages, 0 errors.
- 2026-09-26 (Adeel: make an IEEE Access submission folder). Checked IEEE Access guidelines: LaTeX/Word source AND matching PDF required (<=40 MB), bios for all authors, submitting-author ORCID, 3-10 keywords, AI-generated content must be disclosed in the Acknowledgments. Added \section*{Acknowledgment} with an AI-use disclosure (Claude: analysis/plotting code, drafting and editing text, redrawing Figs. 1 and 4) for the authors to confirm; small trims keep 19 pages. New scripts/make_submission.py builds ../IEEE_Access_Submission/ (outside the repo, holds photos): manuscript PDF; flat LaTeX source + zip that compiles standalone with the official class and matches the PDF text exactly (checked); cover letter .tex/.pdf/.docx (one page, signed by Adeel as submitting author, ORCID 0009-0007-5868-394X from his earlier IEEE Access submission b57487b2, which was a different paper); all 10 figures as PDF + 600-dpi PNG; photos + biographies; submission-form details (title, abstract, keywords, authors, declarations, IEEE-thesaurus keyword suggestions, subject categories); reproducibility zip; checklist.
